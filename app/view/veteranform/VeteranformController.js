Ext.define('Admin.view.veteranform.VeteranformController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.veteranform',
    init: function () {
        ...
        veteranformResults = this.getStore('veteranformResults');
        selectedRecord = null;
        this.control({
            '#': {
                ...
                activate: function (t) {
                    ...
                    if (currentUser.data.length > 0) {
                        ...
                        veteranformResults.getProxy().setExtraParam("ID", currentUser.first().data.Id);
                        veteranformResults.getProxy().setConfig("headers", { 'Authorization': 'Bearer ' + currentToken.first().data.access_token });
                        veteranformResults.load({
                            scope: this,
                            callback: function (records, operation, success) {
                                //console.log(records);
                                if (records.length > 0) {
            ...
            if (currentToken.data.length > 0) {
                atoken = currentToken.first().data.access_token;
                    ...
                    SecurityFormDetailPanel.getForm().submit({
                        ...
                        scope: this,
                        url: 'http://localhost/CorpAppWebServices1/api/corp/AddNewVeteranForm',
                        params: {
